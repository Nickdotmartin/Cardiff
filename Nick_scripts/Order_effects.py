...
for p_name in participant_list:
    if os.path.isdir(os.path.join(exp_path, p_name)):
        print(f"{p_name} exists")
    else:
        print(f"{p_name} does not exist")
...
all_isi_list = [-1, 0, 1, 2, 3, 4, 6, 9, 12, 24]
...
for p_idx, participant_name in enumerate(participant_list):
    print(f"\nrunning participant: {participant_name}")

    root_path = os.path.join(exp_path, participant_name)

    # add flow_type ('flow_dots' or 'flow_rings') to the end of the participant name
    flow_type_list = ['flow_dots', 'flow_rings']
    for flow_type in flow_type_list:
        print(f"\nchecking flow_type: {flow_type}")
        root_path = os.path.join(exp_path, participant_name)

        if not os.path.isdir(os.path.join(root_path, flow_type)):
            print(f"{flow_type} does not exist in {root_path}")
            continue
        else:
            print(f"found {flow_type} in {root_path}")
            root_path = os.path.join(root_path, flow_type)


            # add prelim_duration ('bg70' or 'bg350') after flow type.
            prelim_durs = ['bg70', 'bg350']
            for prelim_dur in prelim_durs:
                print(f"\nchecking prelim_dur: {prelim_dur}")
                root_path = os.path.join(exp_path, participant_name, flow_type)

                if not os.path.isdir(os.path.join(root_path, prelim_dur)):
                    print(f"{prelim_dur} does not exist in {root_path}")
                    continue
                else:
                    print(f"found {prelim_dur} in {root_path}")
                    root_path = os.path.join(root_path, prelim_dur)
