...
exp_path = r'C:\Users\sapnm4\OneDrive - Cardiff University\PycharmProjects\Cardiff\rad_flow_2_delete'
...
for p_idx, participant_name in enumerate(participant_list):
    ...
    for run_idx, run_dir in enumerate(run_folder_names):
        ...
        # sort lists so that neg_sep_vals is in order [18, -18, 6, -6, 3, -3 etc.]
        # get indices to sort neg_sep_vals_list in order sorted(abs(neg_sep_vals_list), reverse=True)

        # sorted_neg_sep_indices = sorted(range(len(neg_sep_vals_list)),
        #                                       key=lambda k: abs(neg_sep_vals_list[k]),
        #                                       reverse=True)
        # print(f"\nsorted_neg_sep_indices: {sorted_neg_sep_indices}")
        ...
        # stair_list_sorted = itemgetter(*sorted_neg_sep_indices)(stair_list)
        # sep_vals_list_sorted = itemgetter(*sorted_neg_sep_indices)(sep_vals_list)
        # neg_sep_vals_list_sorted = itemgetter(*sorted_neg_sep_indices)(neg_sep_vals_list)
        # cong_vals_list_sorted = itemgetter(*sorted_neg_sep_indices)(cong_vals_list)
        # print(f"\nstair_list_sorted: {stair_list_sorted}")
