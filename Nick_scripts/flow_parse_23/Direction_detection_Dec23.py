...
# these setting match this experiment to rad_flow_Dec23.py, but with multiple targets.
...
'''
# it will try to run all of these, but will remove any that are not possible on this monitor
...
# get the actual ms times on this monitor, rounded to int or 2dp (these are only used as labels)
...
# get the equivalent ISI_ms condition for each probe_dur, rounded to int or 2dp (these are only used as labels)
...
# get the values corresponding to each condition (values will be repeated)
...
# There is a different start speed for each condition, such that all conditions
# move the same distance across the number of frames for that condition
start_pix_per_fr_list = [start_dist_pix_in_dur / i for i in probe_dur_fr_conds_list]  # starting dist in pixels per frame
if debug:
    print(f"start_pix_per_fr_list: {start_pix_per_fr_list}")
    print(f"probe_dur_fr_conds_list: {probe_dur_fr_conds_list}")
...
for stair_idx in stair_idx_list:
    ...
    stairStart = start_pix_per_fr_list[stair_idx]


    thisStair = Staircase(name=stair_names_list[stair_idx],
                          type=k_type,  # step size changes after each reversal only
...
for step in range(n_trials_per_stair):
    ...
    for thisStair in stairs:
        ...
        while repeat:
            ...
            so there will be one frame of bg_motion before the probe and one frame of bg_motion after the probe.
            if the window was 100 frames then there would be 46 frames of bg_motion before the probe and 46 frames after the probe.'''
