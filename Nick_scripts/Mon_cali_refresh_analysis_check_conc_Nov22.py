...
This script was to specifically look at why there was a funny result for concurrent probes (pixels stayed on too long).
I tries a couple of methods to fix this.
They all worked.  
...
#                 vid_name = f'ISI_{isi}_sep_{sep}_{cond}_v{ver}.mp4'
...
#                 else:
#                     print(f'\n\tfound: {vid_name}')
#                     details_list.append([isi, sep, ver, cond, vid_name])
#
# excel_headers = ['isi', 'sep', 'version', 'cond', 'filename']
...
=IF(A2=-1, G2, IF(A2=0, G2+8, IF(A2=2, G2+16, IF(A2=3, G2+20, IF(A2=4, G2+24, IF(A2=6, G2+32, IF(A2=9, G2+44, IF(A2=12, G2+56, IF(A2=24, G2+104, 0)))))))))
...
# image_name = f"ISI_{isi}_sep_{sep}_{cond}_v{ver}_fr{show_frame}.jpg"
#
...
# image2_name = f"ISI_{isi}_sep_{sep}_{cond}_v{ver}_fr{p2_show_frame}.jpg"
