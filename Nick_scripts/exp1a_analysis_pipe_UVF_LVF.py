...
for p_idx, participant_name in enumerate(participant_list):
    ...
    # all_data_df['VF'] = visual_field_list
    # # if verbose:
    # print(f"all_data_df:\n{all_data_df}")
    #
    # # if save_all_data:
    ...
    psig_save_name = 'psignifit_both_vfs.csv'

    # sep_list = sorted(all_data_df['separation'].to_list())
    # isi_list = sorted(all_data_df['ISI'].to_list())
    # hue_list = sorted(all_data_df['VF'].to_list())
    #
    # get_psig_thr_w_hue(root_path, p_run_name=participant_name,
    #                    output_df=all_data_df,
    #                    n_bins=25, q_bins=True,
    #                    thr_col='probeLum',
    #                    sep_col='separation', sep_list=sep_list,
    #                    isi_col='ISI', isi_list=isi_list,
    #                    hue_col='VF', hue_list=hue_list,
    #                    trial_correct_col='trial_response',
    #                    conf_int=True, thr_type='Bayes',
    #                    plot_both_curves=False,
    #                    cols_to_add_dict=None, save_name=psig_save_name,
    ...
    UVF_df.to_csv(save_csv_path, index=False)
    ...
    for idx, vis_field_df in enumerate(vis_field_list):
        ...
        sep_list = sorted(vis_field_df['separation'].to_list())
        # isi_list = [-1, 0, 2, 4, 6, 9, 12, 24]
        isi_list = sorted(vis_field_df['ISI'].to_list())
