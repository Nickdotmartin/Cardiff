...
'''
Script to test stimuli durations (by filming them) on Exp1_Jan23_radial_v4:
This script also has a third 'reference' probe which is always displayed at max luminance,and can be
used to detect any change in camera brightness due to adaptation.
'''
...
expName = 'Exp1_Jan23_radial_v4_stimTest'  # from the Builder filename that created this script
...
# 3rd probe to act as reference for luminance
ref_probe = visual.ShapeStim(win, vertices=probeVert, fillColor='white',
                              lineWidth=0, opacity=1, size=probe_size, interpolate=False)
...
for step in range(n_trials_per_stair):
    ...
    for thisStair in stairs:
        ...
        while repeat:
            ...
            ref_probe.setPos([loc_x - 40, loc_y - 20])
            ...
            while continueRoutine:
                ...
                if t_fixation >= frameN > 0:
                    ...
                    ref_probe.draw()
                    frame_counter.text = f"{frameN}, ({actual_trials_inc_rpt}) trial_number: {trial_number}, " \
                                         f"stair_idx: {stair_idx}, thisStair: {thisStair}, " \
