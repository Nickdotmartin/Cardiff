...
el.open() #initiate it
el.sendCommand('sample_rate 500') #sample rate
el.sendCommand('select_parser_configuration 1') #0 = offline analysis, 1 = online analysis (i.e. gaze based experiment where you need to use the gaze in real time)
el.sendCommand("saccade_velocity_threshold = 75") #sets threshold for offline EDF saccade analysis
el.sendCommand("saccade_acceleration_threshold = 9000")
...
Select the separation and ISI conditions you want to test but remember they multiply!
The number of staircases is separations * ISI_values, with 25 trials per staircase.
'''
...
save_output_name = "%s_incomplete"%(filename)
...
if monitor_name in ['ASUS_2_13_240Hz', 'asus_cal', 'Nick_work_laptop', 'NickMac', 'Cog Lab 5 Monitor']:
    display_number = 0
...
el.doTrackerSetup(width=widthPix, height=heightPix)
...
el.startRecording(1,1,1,1) #start recording
...
#                    trials_counter.text = f"{trial_number}/{total_n_trials}"
                ...
                if frameN == 0:
                    ...
                    el.sendMessage("Start fixation %d"%fix_time)
                elif frameN == t_fixation:
                    ...
                    # log el end of stimuli presentation
                    end_time = el.trackerTime()
                    el.sendMessage("End stimulus %d"%end_time)
