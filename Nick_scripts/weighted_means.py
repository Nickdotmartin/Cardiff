...
for participant_name in participant_list:
    ...
    for conf_type in conf_type_list:
        ...
        # # # part 2 # loop through conditions and get weighted means
        ...
        weighted_std_m_array = np.zeros(shape=[len(sep_list), len(isi_name_list)])
        ...
        for isi_idx, isi_name in enumerate(isi_name_list):
            ...
            for sep_idx, sep in enumerate(sep_list):
                ...
                weighted_std_m_array[sep_idx, isi_idx] = weighted_stats.std_mean  # is this really SE or weighted std?
        ...
        weighted_std_m_df = pd.DataFrame(weighted_std_m_array, columns=isi_name_list)
        weighted_std_m_df.insert(0, 'separation', sep_list)

        if conf_type == 'CI_width':
            MASTER_df_name = 'MASTER_WM_CI_thr.csv'
            MASTER_std_df_name = 'MASTER_WM_CI_std.csv'
            MASTER_std_m_df_name = 'MASTER_WM_CI_se.csv'

        elif conf_type == 'eta':
            MASTER_df_name = 'MASTER_WM_eta_thr.csv'
            MASTER_std_df_name = 'MASTER_WM_eta_std.csv'
            MASTER_std_m_df_name = 'MASTER_WM_eta_se.csv'
        ...
        weighted_std_m_df.to_csv(os.path.join(root_path, MASTER_std_m_df_name), index=False)
...
print("finished running weighted means")
