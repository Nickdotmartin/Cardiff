...
rad_flow_v3_missing_probe.py - 1st Nov 23
(same changes as for rad_flow_v3_motion_window.py)
...
Missing_target_detection_Dec23.py version to use for experiment. (matched with Target_detection_Dec23.py)
...
'''
# I've deleted the conditions I don't need (incoherent, rotation, translation)
probes_dict = {'type': {
...
orientation = 'radial'  # probe directions are coherent, eg, all probe 2s is out from probe 1s
probe_coherence = 'radial'  # probe directions are coherent, eg, all probe 2s is out from probe 1s
exp_type = 'bg_congruence' #  probe direction (inward/outward) is either congruent or incongruent with flow direction (contract/expand)
...
# These options are from when probes could be coherent or incoherent, but now they are always coherent.
# keep them here for now.
stair_per_dir = True  # stairs are for cnogruent/incongruent, not inward outward probes.
target_jump_vals = ['mixed']  # if exp_type == 'bg_congruence':
...
else:  # don't include congruence if no background
...
for step in range(n_trials_per_stair):
    ...
    for thisStair in stairs:
        ...
        while repeat:
            ...
            # # # MOTION WINDOW # # #
            '''Rather than just having preliminary motion (before probe1), we now have a motion window of a fixed duration.
            ...
            while continueRoutine:
                ...
                # # # PROBE 1 - after prelim bg_motion, before ISI # # #
