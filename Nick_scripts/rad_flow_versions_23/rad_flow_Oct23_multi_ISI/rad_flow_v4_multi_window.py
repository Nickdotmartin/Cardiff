...
rad_flow_v4_multi_window.py - 1st Nov 23
...
- copied changes from rad_flow_v3_motion_window
...
for this_zip in isi_selected_zip:
    ...
    if act_ms_int == -1:  # concurrent
        ...
        if name_int == act_ms_int:
            ...
            print(f"dropping ISI: {this_zip[2]}, as it is not possible on a monitor that is {round(1000/fps, 2)}ms per frame.")
...
if debug:
    selected_bg_motion_ms_vals = [1000]



'''
If bg_motion_ms_vals contains 0, there will be no background motion at all.
Other than zero, the shorted allowed bg_motion_ms is equal to the longest valid ISI plus both probes.
In situation there would be no prelimiary or post stim motion, but motion during the entire stimulus presentation.
The reason is that the script currently only separates motion into prelim, stim and post (stim = probe1, ISI, probe2).
For shorter ISIs, this same bg_motion_ms would have motion during the probes and ISI, as well as prelim and post stim motion.
Top simplify the conversion between ms and frames, I will do the comparisson on frames.
'''
probe_dur_ms = probe_duration * 1000 / fps
max_stim_dur_ms = max([i[0] for i in isi_zip]) + (probe_dur_ms * 2)
max_stim_dur_fr = int(round(max_stim_dur_ms * fps / 1000, 0))
print(f"\nprobe_dur_ms: {round(probe_dur_ms, 2)}, max_isi_ms: {max([i[0] for i in isi_zip])}, "
      f"max_stim_dur_ms: {round(max_stim_dur_ms, 2)}, max_stim_dur_fr: {max_stim_dur_fr}")

# convert ms values to frames
selected_bg_motion_fr_vals = [-1 if i == -1 else int(i * fps / 1000) for i in selected_bg_motion_ms_vals]
print(f"selected_bg_motion_fr_vals: {selected_bg_motion_fr_vals}")


# change any selected_bg_motion_fr_vals less than max_stim_dur_fr (but greater than zero) to max_stim_dur_fr
if any([i > 0 and i < max_stim_dur_fr for i in selected_bg_motion_fr_vals]):
    print(f"\tWARNING!: selected_bg_motion_fr_vals contains values "
          f"({[i for i in selected_bg_motion_fr_vals if 0 < i < max_stim_dur_fr]}) that are greater than zero, "
          f"but less than max_stim_dur_fr ({max_stim_dur_fr}fr ({round(max_stim_dur_ms, 2)}ms)):  "
          "\nthese values will be changed to match max_stim_dur_fr, so that the background motion is the same duration as the longest stimulus (p1, ISI, p2).")
    # bg_motion_fr_vals = [max_stim_dur_fr if i > 0 and i < max_stim_dur_fr else i for i in bg_motion_fr_vals]
    bg_motion_fr_vals = [max_stim_dur_fr if 0 < i < max_stim_dur_fr else i for i in selected_bg_motion_fr_vals]
else:
    bg_motion_fr_vals = selected_bg_motion_fr_vals

# remove any duplicate values
bg_motion_fr_vals = sorted(list(set(bg_motion_fr_vals)))
print(f"bg_motion_fr_vals: {bg_motion_fr_vals}")

if len(bg_motion_fr_vals) < len(selected_bg_motion_fr_vals):
    print(f"\n\tWARNING!: "
          f"{len(selected_bg_motion_fr_vals) - len(bg_motion_fr_vals)} selected_bg_motion_ms_vals are not possible on this monitor ({monitor_name})"
          f" and they have been dropped.")
print(f"\n(actual) bg_motion_fr_vals: {bg_motion_fr_vals}")


# bg_motion_ms_vals = list(set([-1 if i == -1 else (1 / fps) * i * 1000 for i in bg_motion_fr_vals]))
bg_motion_ms_vals = [-1 if i == -1 else (1 / fps) * i * 1000 for i in bg_motion_fr_vals]
print(f"bg_motion_ms_vals: {bg_motion_ms_vals}")


bg_motion_zip = list(zip(bg_motion_ms_vals, bg_motion_fr_vals))
print(f"bg_motion_zip: {bg_motion_zip}")
...
if debug:
    print(f'\ncongruence_vals: {congruence_vals}')
    print(f'congruence_names: {congruence_names}')
    print(f"cong_zip: {cong_zip}")



# get all possible combinations of isi_zip, cong_zip and bg_motion_zip
combined_conds = [(iz, cz, bz) for iz in isi_zip for cz in cong_zip for bz in bg_motion_zip]
print(f"\ncombined_conds: {combined_conds}")

# unpack combined conds to get variable values for each cond.
...
bg_motion_ms_conds_list = [i[2][0] for i in combined_conds]
bg_motion_fr_conds_list = [i[2][1] for i in combined_conds]
...
if debug:
    print(f'\nisi_ms_conds_list: {isi_ms_conds_list}')
    print(f'isi_fr_conds_list: {isi_fr_conds_list}')
    print(f'isi_name_conds_list: {isi_name_conds_list}')
    ...
    print(f'bg_motion_fr_conds_list: {bg_motion_fr_conds_list}')
    print(f'bg_motion_cond_names_list: {bg_motion_cond_names_list}')


# get names for staircases
stair_names_list = [f"ISI_{i}_{cn}_{cv}_bg_motion_{b}" for i, cn, cv, b in
                    zip(isi_name_conds_list, cong_name_conds_list, cong_val_conds_list, bg_motion_cond_names_list)]
if background == 'no_bg':
    stair_names_list = [f"ISI_{i}_{cn}_{cv}" for i, cn, cv in zip(isi_name_conds_list, cong_name_conds_list, cong_val_conds_list)]
...
print(f"\nbgLumProp: {bgLumProp}; bgLum: {bgLum}; bgColor_rgb1: {bgColor_rgb1}")
...
for stair_idx in range(n_stairs):
    ...
    thisInfo['ISI_name'] = isi_name_conds_list[stair_idx]
...
for step in range(n_trials_per_stair):
    ...
    for thisStair in stairs:
        ...
        while repeat:
            ...
            isi_name = thisStair.extraInfo['ISI_name']
            ...
            bg_motion_fr = thisStair.extraInfo['bg_motion_fr']
            bg_motion_name = thisStair.extraInfo['bg_motion_name']
            # prelim_ms = thisStair.extraInfo['prelim_ms']
            if debug:
                print(f"sep: {sep}\n"
                      f"isi_name: {isi_name}, isi_ms: {isi_ms}, isi_cond_fr: {isi_cond_fr}, \n"
                      f"cong_name: {cong_name}, congruent: {congruent}, \n"
                      f"bg_motion_name: {bg_motion_name}, bg_motion_ms: {bg_motion_ms}, bg_motion_fr: {bg_motion_fr}, \n"
                      )
            ...
            # Get the duration (number of frames) for probes and ISI
            ...
            if isi_cond_fr < 0:  # -1 means probes 1 and 2 are presented concurrently with no isi
            ...
            # frame number marking the end of each segment (or cumulative timing of experiment)
            end_fix_fr = int(fps / 2) + vary_fix - prelim_dur_fr  # end of fixation
            if end_fix_fr < 0:
                end_fix_fr = int(fps / 2)
            end_prelim_fr = end_fix_fr + prelim_dur_fr  # end of preliminary motion
            end_p1_fr = end_prelim_fr + probe_duration  # end of probe 1
            end_ISI_fr = end_p1_fr + isi_dur_fr  # end of ISI
            end_p2_fr = end_ISI_fr + p2_dur_fr  # end of probe 2
            end_post_fr = end_p2_fr + post_dur_fr  # end of post-probe2 bg_motion
            if debug:
